 ...
 * @param {object} gameData - The static game data object.
 ...
 */
function processAction(gameData, currentState, currentMap, actionCommand, actingFaction) {
    ...
    switch (command) {
        case 'construir':
            return build(gameData, newState, newMap, factionInNewState, parts);
...
function build(gameData, state, map, faction, parts) {
    if (parts.length < 3) {
